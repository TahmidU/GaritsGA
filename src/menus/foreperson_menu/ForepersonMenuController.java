...
import menus.mechanic_menu.UpdateJobController;
 ...
 * @author Huntees
 */
public class ForepersonMenuController implements Initializable {
    ...
    StaffDAO sDAO;
    ObservableList<Staff> mechanicData;

    @FXML
    private Label loggedInAsText;
    @FXML
    private TabPane forepersonTab;
    ...
    -----------------------------------------------Mechanic Option-------------------------------------------------------------------
         */
        sDAO = new StaffDAO();
        mechanicData = FXCollections.observableArrayList(sDAO.getByType("Mechanic"));
        
        ObservableList<String> mechanicNames = FXCollections.observableArrayList();
        for (int i = 0; i < mechanicData.size(); i++) {
            mechanicNames.add(mechanicData.get(i).getUserName());
        }
        
        mechanicCombo.setItems(mechanicNames);
    }

    public void switchTab(int n) {
        SingleSelectionModel<Tab> selectionModel = forepersonTab.getSelectionModel();
...
//=================================================================================================================================
    ...
    private void pickJobPress(ActionEvent event) throws IOException {
        ...
        if (selectedJob == null) {
            ...
            if (selectedJob.getStaff() == null) {
                ...
                MainGUIController guiController = new MainGUIController();
                guiController.popupConfirmation(event, "Are you sure you want to remove this mechanic from this job?");
    ...
    @FXML
    private void allocateMechanicPress(ActionEvent event) throws IOException {
        JobSheet selectedJob = null;
        selectedJob = jobTable.getSelectionModel().getSelectedItem();
        ...
        } else if (mechanicCombo.getValue() == null) {
            noJobSelected.setText("No Mechanic Selected");

        } else {
            if (selectedJob.getStaff() == null) {
                JobSheet jobTMP = new JobSheet(selectedJob.getJobNum(), sDAO.getByUserName(mechanicCombo.getValue()).getId(),
                        selectedJob.getVehicleReg(), selectedJob.getBookingId(), selectedJob.getProblemDesc(),
                        selectedJob.getDateCreated(), selectedJob.getStatus(), selectedJob.getDateCompleted());
                jsDAO.update(jobTMP);
                jobSuccessful.setText("Allocation Successful!");
            } else {
                MainGUIController guiController = new MainGUIController();
                guiController.popupConfirmation(event, "Are you sure you want to remove this mechanic from this job?");
                ...
                if (guiController.popupController.getConfirm()) {
                    JobSheet jobTMP = new JobSheet(selectedJob.getJobNum(), -1,
                            ...
                            selectedJob.getDateCreated(), selectedJob.getStatus(), selectedJob.getDateCompleted());
                    jsDAO.update(jobTMP);
                    jobSuccessful.setText("Remove Successful!");
                }
            }
            noJobSelected.setText("");
            refreshJobTable();
